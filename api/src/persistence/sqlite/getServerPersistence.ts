...
export const getServerPersistence = async ({ applicationContext, serverId }: getServerPersistenceOptions) => {
  const servers = await (await applicationContext.db).all(
    'SELECT s.id as serverId, n.id as nodeId, s.restart, s.runBackup, s.memoryPercent, s.cpuPercent, s.memory, s.running, s.port, n.ip from `servers` as s JOIN `nodes` as n ON s.`nodeId` = n.`id` WHERE `serverId` = ?',
    [serverId],
  );
